...
class RBFKernelFunction(KernelFunction):
    ...
    def __compute_dist(self, X: Variable, X_prime: np.ndarray,
        ...
        X_tr: OnnxOperator = OnnxTranspose(X, op_version=op_version)
        Y_tr: OnnxOperator = OnnxTranspose(Y, op_version=op_version)
        X_tr_X: OnnxOperator = OnnxMatMul(X_tr, X, op_version=op_version)
        Y_tr_Y: OnnxOperator = OnnxMatMul(Y_tr, Y, op_version=op_version)
        X_tr_Y: OnnxOperator = OnnxMatMul(X_tr, Y, op_version=op_version)
        neg_two_X_tr_Y: OnnxOperator = OnnxMul(-2.0,
                                               X_tr_Y,
                                               op_version=op_version)
        dist: OnnxOperator = OnnxSum([X_tr_X, neg_two_X_tr_Y, Y_tr_Y],
                                     op_version=op_version)
        return dist
    ...
    def __call__(self, X: Variable, X_prime: np.ndarray,
        ...
        neg_gamma_dist: OnnxOperator = OnnxMul(-1.0 * gamma,
                                               dist,
                                               op_version=op_version)
...
def rvr_converter(scope: Scope, operator: Operator,
    ...
    op_outputs: List[Variable] = operator.outputs
    phi_matrix: OnnxOperator = kernel_func(input, rvr.relevance_vectors_,
                                           op_version)
    y: OnnxOperator = OnnxMatMul(phi_matrix,
                                 rvr.weight_posterior_mean_,
                                 op_version=op_version,
